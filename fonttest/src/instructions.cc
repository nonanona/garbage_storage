...
#define FOR_EACH_INSTRUCTIONS(V) \
    ...
    V(WCVTF, 0, 0x70, zero) \
    V(DELTAC1, 0, 0x73, zero) \
    V(DELTAC2, 0, 0x74, zero) \
    V(DELTAC3, 0, 0x75, zero) \
...
void DELTAC1(int opcode, const uint8_t* is, size_t len, Context* ctx, int* pc) {
  LOG(ERROR) << __FUNCTION__;
  LOG(FATAL) << "Not implemented.";
}

void DELTAC2(int opcode, const uint8_t* is, size_t len, Context* ctx, int* pc) {
  LOG(ERROR) << __FUNCTION__;
  LOG(FATAL) << "Not implemented.";
}

void DELTAC3(int opcode, const uint8_t* is, size_t len, Context* ctx, int* pc) {
  LOG(ERROR) << __FUNCTION__;
  LOG(FATAL) << "Not implemented.";
}
